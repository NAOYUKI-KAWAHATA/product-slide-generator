...
| 4 | `geometric` | Gemini風グラデーション | ホワイトグレー＋幾何学線 | ブルー→パープル→ピンクのグラデーション | AI・テック・ビジネス |
| 5 | `soft_beige` | イエローベージュ | ソフトなイエローベージュ | ベージュグラデーション | ライフスタイル、子育て、健康 |
...
## 全テーマ共通：文字数過多・段ズレ・レイアウト崩れ防止ルール

スライドの品質を保つため、テキスト量とレイアウトバランスに関して以下のルールをすべてのテーマで**必ず遵守**すること。

### ページ分割の目安

| テンプレート | 1ページの最大量 | 超えた場合の対応 |
|---|---|---|
| 02_bullet_arrow / 03_bullet_list（箇条書き） | 1ページあたり最大3項目 | 4項目以上は2ページに分割 |
| 07_phase_card（フェーズカード） | 1ページあたり最大3フェーズ | 4フェーズ以上は2ページに分割 |
| 08_feature_card / 09_feature_card_detail（特徴カード） | 1ページあたり最大3〜4カード | 超えた場合は2ページに分割 |
| 10_content_summary（提供コンテンツ） | 1ページあたり最大4項目 | 超えた場合は2ページに分割 |

ページ分割する場合は `slide_organize` で新しいページを追加し、内容を意味の区切りで自然に振り分けること。

### 適切な改行とレイアウト保護

- テキストが長い場合は、意味の区切りで `<br>` を使って手動改行を入れ、自然な折り返しを制御すること。
- テキストエリアには必ず `max-width` または固定幅を設定し、装飾要素（バナー・フッター帯・波形装飾）にテキストが被らないようにすること。
- 行数が増えても下部のフッター帯・波形装飾・バナーと重ならないよう、コンテンツエリアの `padding-bottom` を適切に設定すること（目安: `padding-bottom: 80px` 以上）。

### 全体バランスの最適化

- コンテンツが上部や左側に偏らないよう、スライド全体（1280×720px）に対して中央寄りの美しいバランスを保つこと。
- `display: flex; flex-direction: column; justify-content: center;` または `align-items: center;` を活用し、縦横方向の余白を均等に保つこと。
- テキストが少ない場合でも、上下左右に十分な余白（`padding-top: 40px` 以上）を確保し、窮屈に見えないようにすること。
- 生成後に「上に偏っている」「左に寄りすぎている」と感じたら、`justify-content` や `padding` を調整して中央寄りに修正すること。
